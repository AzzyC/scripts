...
cd ..
telegram -MD "Syncing [Bootleggers 5.0-p](https://github.com/BootleggersROM/manifest/tree/queso)"
syncstart=`date +%s`
...
telegram -MD "Sync completed in "$synctimeM" minutes or "$synctimeS" seconds, for [Bootleggers 5.0-p](https://github.com/BootleggersROM/manifest/tree/queso)"
...
telegram -MD "Build started for Starlte, [Bootleggers 5.0-p](https://github.com/BootleggersROM/manifest/tree/queso)"
...
telegram -MD "Build started for Star2lte, [Bootleggers 5.0-p](https://github.com/BootleggersROM/manifest/tree/queso)"
...
if compgen -G "$rom_out/star2lte/BootleggersROM-Queue4*.zip" > /dev/null; then
	mv $rom_out/star*/BootleggersROM-Queue4*.zip ~/permissive/
	mv $rom_out/star*/BootleggersROM-Queue4*.zip.md5sum ~/permissive/
fi
wget https://github.com/gdrive-org/gdrive/releases/download/2.1.0/gdrive-linux-x64
chmod +x gdrive-linux-x64
sudo install gdrive-linux-x64 /usr/local/bin/gdrive
gdrive upload -p 1qp133uQXFNur6tKqbs251uJ5CLCUxBgI -r permissive
