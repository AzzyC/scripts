...
mkdir rom/ && cd rom/
repo init -u git://github.com/crdroidandroid/android.git -b 10.0 --no-clone-bundle --depth=1
cd .repo/
git clone https://github.com/AzzyC/local_manifests.git -b android-10.0 --depth=1
cd ..
telegram -MD "Syncing [crDroid 6.4](https://github.com/crdroidandroid/android/tree/10.0)"
syncstart=`date +%s`
...
telegram -M "Sync completed in "$synctimeM" minutes or "$synctimeS" seconds, for [crDroid 6.4](https://github.com/crdroidandroid/android/tree/10.0)"
. build/envsetup.sh
lunch lineage_starlte-userdebug
telegram -MD "Build started for Starlte, [crDroid 6.4](https://github.com/crdroidandroid/android/tree/10.0)"
...
if compgen -G "$rom_out/starlte/crDroidAndroid-10.0-202*.zip" > /dev/null; then
...
telegram -MD "Build started for Star2lte, [crDroid 6.4](https://github.com/crdroidandroid/android/tree/10.0)"
...
mkdir $date/
sleep 2
if compgen -G "$rom_out/star2lte/crDroidAndroid-10.0-202*.zip" > /dev/null; then
	mv $rom_out/star*/crDroidAndroid-10.0-202*.zip ~/$date/
	mv $rom_out/star*/crDroidAndroid-10.0-202*.zip.md5sum ~/$date/
fi
